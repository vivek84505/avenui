...
function ajax_preview_modal(type,title,id){
    preview_modal(title);
    ajax_load(base_url + '' + user_type + '/' + module + '/preview/' + type + '/' + id, 'form', 'form');
    sound('ajax_load');
}
...
function form_submit(form_id, noty, e) {
    ...
    form.find(".required").each(function () {
        ...
        var here = $(this);
        if (here.val() == '' || here.val() == 'null' || here.val() == null) {
            if (!here.is('select')) {
                here.css({borderColor: 'red'});
                if (here.attr('type') == 'number') {
                    txt = '*' + mbn;
    ...
    window.set_switchery = function () {
        if ($('#prod').length) {
            ...
            $(".sw2").each(function () {
                ...
                changeCheckbox.onchange = function () {
                    //alert($(this).data('id'));
                    ajax_load(base_url + '' + user_type + '/' + module + '/featured_set/' + $(this).data('id') + '/' + changeCheckbox.checked, 'prod', 'others');
                    if (changeCheckbox.checked == true) {
                        $.activeitNoty({
                            type: 'success',
                            icon: 'fa fa-check',
            ...
            $(".sw3").each(function () {
                ...
                changeCheckbox.onchange = function () {
                    //alert($(this).data('id'));
                    ajax_load(base_url + '' + user_type + '/' + module + '/deal_set/' + $(this).data('id') + '/' + changeCheckbox.checked, 'prod', 'others');
                    if (changeCheckbox.checked == true) {
                        $.activeitNoty({
                            type: 'success',
                            icon: 'fa fa-check',
        ...
        else if ($('#payment_set').length) {
            $(".payment_switchery").each(function () {
                var h = $(this);
                var id = h.attr('id');
                var set = h.data('set');
                var true_msg = h.data('tm');
                var false_msg = h.data('fm');
                new Switchery(document.getElementById(id), {color: 'rgb(100, 189, 99)', secondaryColor: '#cc2424', jackSecondaryColor: '#c8ff77'});
                var changeCheckbox = document.querySelector('#' + id);
                changeCheckbox.onchange = function () {
                    ajax_load(base_url + '' + user_type + '/' + module + '/activation/' + set + '/' + changeCheckbox.checked, 'site', 'others');
                    if (changeCheckbox.checked == true) {
                        $.activeitNoty({
                            type: 'success',
                            icon: 'fa fa-check',
        ...
        else if ($('#ad_set').length) {
            $(".ad_switchery").each(function () {
                var h = $(this);
                var id = h.attr('id');
                var set = h.data('set');
                var true_msg = h.data('tm');
                var false_msg = h.data('fm');
                var ajaxa = h.data('ajax');
                new Switchery(document.getElementById(id), {color: 'rgb(100, 189, 99)', secondaryColor: '#cc2424', jackSecondaryColor: '#c8ff77'});
                var changeCheckbox = document.querySelector('#' + id);
                changeCheckbox.onchange = function () {
                    if(ajaxa == 'no'){
                        ...
                        if(ajaxa == undefined){
                            ajax_load(base_url + '' + user_type + '/' + module + '/status/' + set + '/' + changeCheckbox.checked, '', 'others');
                        } else {
                            ajax_load(ajaxa + changeCheckbox.checked, '', 'others');
                        }
                        if (changeCheckbox.checked == true) {
                            $.activeitNoty({
                                type: 'success',
